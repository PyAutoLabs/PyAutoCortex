# Euclid — do we recover Sersic indices? 10 simulated Euclid lenses vs the real prior-edge pile-up

Project: euclid
Summary: Do we recover Sersic indices or is prior pile-up artefact
State: planned
Gates:
Witness: 10 simulated fits complete with a recovered-vs-input Sersic index table and plot, the same 10 real lenses' prior-edge status established from dr1_prelim_10_lens_science_run, and a written verdict naming the likely cause
Budget: 24:00
Runs:
Ruling:
Review-minutes: 20
...
Ready when: `resimulate_fitted_lens_simulator` is accepted.
...
The `resimulate_fitted_lens_simulator` resimulations are built with indices in [2, 4] and without whatever data defect is
...
resimulation replacement-index rule was used and what it costs the inference. Confirm the simulated
...
- The real-lens prior-edge status of the same 10 lenses established from
  `dr1_prelim_10_lens_science_run` output.
...
- `euclid_dr1_prelim` (project row): the simulated datasets and their truth table
