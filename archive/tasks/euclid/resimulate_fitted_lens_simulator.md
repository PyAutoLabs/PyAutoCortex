# Euclid — resimulate a fitted Euclid lens, and resimulate the 10 prelim lenses with true magnifications recorded

Project: euclid
Summary: Can a fitted Euclid lens be resimulated and recovered
State: planned
Gates:
...
Ready when: `euclid_dr1_prelim/dr1_prelim_10_lens_science_run` is accepted (it
needs the 10 real fits as truth inputs).
...
that is explicitly fine — `sersic_index_recovery` tests recovery under a *matched* model first). Record the
true magnification of every simulated lens as a first-class output stored alongside the
dataset — `magnification_robustness`'s entire question depends on that ground truth, so it must not be
...
(fixed 3.0? drawn? population median?) — `sersic_index_recovery`'s interpretation depends on it. Match the
...
(6a and 6b are the Cortex tasks `sersic_index_recovery` and
`magnification_robustness`.)
...
- `tasks/euclid_dr1_prelim/dr1_prelim_10_lens_science_run.md` — the fits these resimulations start from
