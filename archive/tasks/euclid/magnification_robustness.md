# Euclid — how robust are magnification estimates? model-match vs mismatch across the 10 Euclid lenses

Project: euclid
Summary: How robust are magnification estimates under model match and mismatch
State: planned
Gates:
...
Ready when: `resimulate_fitted_lens_simulator` is accepted. Runs in parallel
with `sersic_index_recovery`.
...
The comparison matrix, fitted against the recorded true magnification of the 10
`resimulate_fitted_lens_simulator` simulations (Sersic lens light + Sersic source):
...
the resimulations' recorded truth is the same quantity; a disagreement between the two is itself a finding.
...
- `euclid_dr1_prelim` (project row): the resimulations and their recorded true magnifications
