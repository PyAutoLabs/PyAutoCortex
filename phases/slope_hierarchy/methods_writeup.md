# Slope_hierarchy — phase 2: methods write-up (NUTS headline, EP cautionary)

Project: slope_hierarchy
Phase: 2
State: planned
Gates:
Witness: prose plus figures and tables committed under `paper/` in the slope_hierarchy checkout, every number regenerated from the committed `results/` JSONs rather than hand-copied, and the scope decision (methods note vs publishable short paper) recorded in the file itself
Budget: 8:00
Runs:
Ruling:
Lane: local-dev
...
Epic: graphical-ep
Filed: 2026-07-22
Migrated-from: PyAutoMind/draft/research/graphical_ep/slope_hierarchy_methods_writeup.md

## Question

What does the `slope_hierarchy` project's finished N=5 result actually claim, written down?
...
This phase produces no runs of its own: the runs are done and committed. It is a
write-up phase, and its deliverable is prose, so `Runs:` stays empty and the
ruling is a judgement on the written claim.
...
- a **publishable short paper**, which realistically wants the N=25–50 scale-up
  (phase 1, `phases/slope_hierarchy/n25_scale_up.md`) first, since N=5 is thin
  for a headline parity claim.
...
Prose plus figures and tables committed under `paper/`, with the figures
regenerated from the committed `results/` JSONs (do not hand-copy numbers — they
are all in `results/*.json` and
`results/ep_history_n5_maxsteps12/parent_trajectory.csv`). The scope decision
(note vs paper) is recorded in the written file, not left implicit.

The repo is private and personal — keep the prose and figures out of public
repos.

## Where to look

- `/mnt/c/Users/Jammy/Science/slope_hierarchy/results/`
- `/mnt/c/Users/Jammy/Science/slope_hierarchy/results/ep_history_n5_maxsteps12/parent_trajectory.csv`
- `/mnt/c/Users/Jammy/Science/slope_hierarchy/paper/`
