# Inference_programme — mass_pix gradient cost probe — forward vs value_and_grad on the A100, strict FD on all 7 params

Project: inference_programme
Summary: Is a mass_pix gradient affordable on an A100 and correct
State: dropped
...
1. **The ratio.** `value_and_grad` / forward, measured on this cell. A ratio
   **≲ 4×** is sane and `prodigy_mass_pix` is affordable. A ratio near **17×** means the
   CPU anomaly reproduces on the GPU, and that is a **library bug to file in the
   Mind before `prodigy_mass_pix` runs**, not a cost to accept.
