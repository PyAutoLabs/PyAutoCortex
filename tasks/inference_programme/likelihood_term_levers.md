# Inference_programme — likelihood-term levers on mass_pix — slogdet vs cholesky, relative jitter on reg.Adapt
...
**This task arms only if `prodigy_mass_pix` shows NaN lanes or misses.** If it hits
...
**Ready when:** `prodigy_mass_pix` rules, and its ruling names a NaN or miss population for
