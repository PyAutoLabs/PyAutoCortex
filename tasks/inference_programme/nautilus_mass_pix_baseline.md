# Inference_programme — the Nautilus bar on mass_pix — f1e8 (refs convention) and f1e5 (like-for-like)
...
This is the bar `prodigy_mass_pix` has to beat. Two arms, both n_live 300, seed 0, fp64,
...
- **`pos_tauto0.2_f1e5`** — the *like-for-like* arm. The gradient search in
  `prodigy_mass_pix` must run at 1e5 (Gate B pt 2: on MGE, factor 1e8 gives Prodigy 0/4 and 1e5
...
on this cell, no prior measurement"; it is this task's own Nautilus wall that
becomes the measured basis `prodigy_mass_pix` inherits (the wall basis is measured on the
