# Inference_programme — Phase 5 — mesh global gradient searches with PositionsLH at factor 1e5 (the first mesh science since the rewind)

Project: inference_programme
Summary: Does Prodigy reach the physical basin on mesh cells positions-on
State: dropped
Gates:
...
`refs_v1_positions_on_completion` (RAL 342241) is ruled.
**No `legacy_wrong` number is a bar.** In particular the
...
kernel-CDF ranking rather than parking it. The three cells above still wait on their bars,
which is why this task stays `planned` and gated on
`refs_v1_positions_on_completion` being ruled. The dev leg
...
1. **`refs_v1_positions_on_completion` ruled.** All three in-scope cells take their bar from it; without it the witness
   has no numbers.
...
- `PyAutoCortex/tasks/inference_programme/refs_v1_positions_on_completion.md` — the prerequisite
